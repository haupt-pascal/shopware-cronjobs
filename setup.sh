...
select SW_VERSION in "Shopware 5" "Shopware 6"; do
    case $SW_VERSION in
        "Shopware 5")
            ...
            CACHE_WARM_COMMAND="sw:warm:http:cache"
            break
            ;;
        "Shopware 6")
            SHOPWARE_VERSION=6
            CACHE_CLEAR_COMMAND="cache:clear --env=prod"
            CACHE_WARM_COMMAND="http:cache:warm --env=prod"
            break
            ;;
...
AVAILABLE_CRONJOBS[1]="Cache Clear and Warm"
# Add more cronjobs as needed
# AVAILABLE_CRONJOBS[2]="Database Backup"
# AVAILABLE_CRONJOBS[3]="Sitemap Generation"
...
echo "$(( ${#AVAILABLE_CRONJOBS[@]} + 1 )). More cronjobs will be added in the future"
echo ""

read -p "Enter the number(s) of the cronjobs you want to install (e.g., 1 or 1,2): " SELECTED_JOBS
...
if [[ $SELECTED_JOBS == *"1"* ]]; then
    echo "Installing Cache Clear and Warm cronjob..."
    ...
    echo "Cache Clear and Warm scripts created successfully!"
fi

# Additional cronjobs can be added here in the future
# if [[ $SELECTED_JOBS == *"2"* ]]; then
#     echo "Installing Database Backup cronjob..."
